...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    :param rnn_kwargs: Additional keyword arguments to pass the rnn
        constructor.
    """
    ...
    def __init__(
        ...
        if not (self.shared_rnn or self.enable_critic_rnn):
            self.critic = nn.Linear(self.features_dim, rnn_hidden_size)

        # Use a separate RNN for the critic
        if self.enable_critic_rnn:
            self.rnn_critic = nn.RNN(
                self.features_dim,
                rnn_hidden_size,
                num_layers=n_rnn_layers,
                **self.rnn_kwargs,
    ...
    def _process_sequence(
        features: th.Tensor,
        # ToDo changing the rnn_state type here
        ...
        # todo changing the rnn_states definition here
        :param rnn_states: previous cell and hidden states of the RNN
        ...
        for features, episode_start in zip_strict(features_sequence, episode_starts):
            ...
            #         # ToDo check if rnn_states work here
            #         # (1.0 - episode_start).view(1, n_seq, 1) * lstm_states[0],
            #         (1.0 - episode_start).view(1, n_seq, 1) * rnn_states,
    ...
    def forward(
        ...
        # print('in forward rnn_states is\n: ', rnn_states)
        # print('in forward rnn_states.pi is\n: ', rnn_states.pi)
        ...
        if self.rnn_critic is not None:
            ...
            rnn_states_vf = rnn_states_pi.detach()
        else:
